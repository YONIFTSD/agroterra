  ...
  <div class="w-100">
        <div class="table-responsive mt-3">
              <table  class="table  table-bordered table-hover table-lg mt-lg mb-0">
                ...
                <tbody v-for="(item, it) in quotation_detail" :key="item.id_product">
                  <tr>
...
export default {
  ...
  methods: {
    ...
    DeleteDetail,
    NameUnitMeasure,
    ...mapActions('Quotation',['mLoadEditQuotationDetail']),
    ...mapActions('Quotation',['mLoadDeleteQuotationDetail']),
    
  },
  computed: {
    ...mapState('Quotation',['quotation_detail']),
...
function EditDetail(id_product) {
  let name = '';
  for (let index = 0; index < this.quotation_detail.length; index++) {
    const element = this.quotation_detail[index];
  ...
  this.mLoadEditQuotationDetail(id_product);
...
function DeleteDetail(id_product) {
  ...
  this.mLoadDeleteQuotationDetail(id_product);
  EventBus.$emit('TotalPaymentCash');
  EventBus.$emit('ChangeFeesCollectedModal');
}
</script>
