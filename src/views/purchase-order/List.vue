  ...
  <div>
    <CRow>
      <CCol col>
        <CCard>
          <CCardHeader>
            <strong> Modulo Orden de Compra - Listar</strong>
          </CCardHeader>
          <CCardBody>
            <b-row>
                ...
                <b-col sm="6" md="1">
                  <b-form-group label=".">
                    <b-link  v-if="Permission('PurchaseOrderAdd')" class="btn form-control btn-primary"  :to="{ path: '/orden-de-compra/nuevo' }" append ><i class="fas fa-file"></i></b-link>
                  </b-form-group>
                </b-col>
                  ...
                  <tr>
                    ...
                    <td class="text-center">
                      <b-dropdown bloque size="sm" text="Acciones" right>
                        <b-dropdown-item v-if="Permission('PurchaseOrderEdit')" @click="EditShopping(item.id_purchase_order)" >Editar</b-dropdown-item>
                        <b-dropdown-item v-if="Permission('PurchaseOrderView')"  @click="ViewShopping(item.id_purchase_order)">Ver</b-dropdown-item>
                        <b-dropdown-item v-if="Permission('PurchaseOrderEdit')"  @click="SendEmail(item.id_purchase_order)">Enviar Email</b-dropdown-item>
                        
                        <b-dropdown-item v-if="Permission('PurchaseOrderDelete')" @click="ConfirmDeleteShopping(item.id_purchase_order)">Eliminar</b-dropdown-item>
                      </b-dropdown>
                    </td>
                  </tr>
                </tbody>
...
function ListShopping() {
  ...
  let me = this;
  let url = this.url_base + "purchase-orders/list/" + id_provider + "/" + this.from + "/" + this.to + "/" + this.id_establishment + "/" + search + "?page=" + this.currentPage;
...
// Editar usuario
function EditShopping(id_purchase_order) {
  this.$router.push({
    name: "PurchaseOrderEdit",
    params: { id_purchase_order: je.encrypt(id_purchase_order) },
  });
}

// Ver Usuario
function ViewShopping(id_purchase_order) {
  this.$router.push({
...
// Confirmar eliminar
function ConfirmDeleteShopping(id_purchase_order) {
  Swal.fire({
    ...
    if (result.value) {
      this.DeleteShopping(id_purchase_order);
    }
  });
}

// eliminar usuario
function DeleteShopping(id_purchase_order) {
  let me = this;
  let url = this.url_base + "purchase-orders/delete/" + id_purchase_order;
  axios({
    method: "delete",
    url: url,
    headers: {
    ...
    .then(function (response) {
      if (response.data.status == 200) {
        ...
        for (var i = 0; i < me.data_table.length; i++) {
          if (me.data_table[i].id_purchase_order == id_purchase_order) {
            me.data_table.splice(i, 1);
            break;
          }
        }
        Swal.fire({ icon: 'success', text: 'Se ha eliminado la orden de compra', timer: 2000, })
...
function SendEmail(id_purchase_order) {
  let me = this;
  let url = this.url_base + "purchase-orders/send-email/" + id_purchase_order;
  axios({
    method: "get",
    url: url,
    headers: {
    ...
    .then(function (response) {
      if (response.data.status == 200) {
        Swal.fire({ icon: 'success', text: 'Se ha enviado el email correctamente', timer: 2000, })
      }else {
        Swal.fire({
          icon: 'error',
          text: 'A ocurrido un error',
