...
export default {
  ...
  methods: {
    ...
    'mLoadAddSaleDetail','mLoadEditCoin','mLoadAddLinkages','mLoadTotalSaleDetail','mLoadIgvPercentage']),
  },
  ...
  axios({
    ...
    .then(function (response) {
      if (response.data.status == 200) {
         ...
         let order = {
          module:'Quotation',
          id_module:response.data.result.quotation.id_quotation,
          concept:'Cotización',
          broadcast_date:response.data.result.quotation.broadcast_date,
          reference: me.CodeInvoice(response.data.result.quotation.type_invoice) +  " " +response.data.result.quotation.serie + " - "+ response.data.result.quotation.number,
        };
