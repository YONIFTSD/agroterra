            ...
            <table class="table table-hover table-bordered">
              ...
              <tbody v-for="(item, it) in account_pays" :key="it">
                <tr>
                  ...
                  <td class="text-center">
                      <button type="button" @click="AddAccountPay(it)" class="btn btn-info">
...
export default {
  ...
  methods: {
      ListAccountPay,
      AddAccountPay,
      ...mapActions('SettlementPayment',['mLoadAddAccountPay']),
  },
  computed: {
    ...mapState(["url_base"]),
    token: function () {
...
function AddAccountPay(index) {
  
    let total = this.$refs['mAmount'+index][0]['value'];
    let me = this;
    let account = this.account_pays[index];
    ...
    if (parseFloat(total) <= parseFloat(account.balance)) {
      ...
      me.mLoadAddAccountPay(account_pay);
    }else{
...
function ListAccountPay() {
  let me = this;
  let url = this.url_base + "settlement-payment/list-by-provider/"+this.id_establishment +"/"+this.id_provider+"/"+this.coin;
  axios({
    method: "GET",
    url: url,
