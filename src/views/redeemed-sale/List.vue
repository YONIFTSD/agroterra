  ...
  <div>
    <CRow>
      <CCol col>
        <CCard>
          <CCardHeader>
            <strong> Modulo de Canje de Venta - Listar</strong>
          </CCardHeader>
          <CCardBody>
              ...
              <b-row>
                ...
                <b-col sm="6" md="1">
                  <b-form-group label=".">
                    <b-link  v-if="Permission('RedeemedSaleAdd')" class="btn form-control btn-primary"  :to="{ path: '/canje-venta/nuevo' }" append ><i class="fas fa-file"></i></b-link>
                  </b-form-group>
                </b-col>
                  ...
                  <tr>
                    ...
                    <td class="text-center">
                      <b-dropdown bloque size="sm" text="Acciones" right>
                        <b-dropdown-item v-if="Permission('SaleEdit')  && (item.state == 1 || item.state == 3)" @click="EditSale(item.id_redeemed_sale)">Editar</b-dropdown-item>
                        <b-dropdown-item v-if="Permission('SaleView')"  @click="ViewSale(item.id_redeemed_sale)">Ver</b-dropdown-item>
                        <b-dropdown-item v-if="Permission('SaleDelete') && (item.state == 1 || item.state == 3)" @click="ConfirmDeleteSale(item.id_redeemed_sale)">Eliminar</b-dropdown-item>
                        <b-dropdown-item v-if="item.state == 3 || item.state == 5" @click="SendXML(item.id_redeemed_sale)">Enviar XML</b-dropdown-item>
                        <b-dropdown-item v-if="item.state == 4" @click="modalCPESunat(item.id_redeemed_sale)">Ver CPE</b-dropdown-item>
                      </b-dropdown>
                    </td>
                  </tr>
                </tbody>
...
function ListSales() {
  ...
  let me = this;
  let url = this.url_base + "redeemed-sale/list/" + id_client + "/" + this.from + "/" + this.to + "/" + this.id_establishment + "/" + search + "?page=" + this.currentPage;
...
// Editar usuario
function EditSale(id_redeemed_sale) {
  this.$router.push({
    name: "RedeemedSaleEdit",
    params: { id_redeemed_sale: je.encrypt(id_redeemed_sale) },
  });
}

// Ver Usuario
function ViewSale(id_redeemed_sale) {
  this.$router.push({
...
function AddNotaCreditoDebito(id_redeemed_sale) {
  this.$router.push({
...
function ViewReferralGuide(id_redeemed_sale) {
  EventBus.$emit('ModalReferralGuideShow',id_redeemed_sale);
}
function modalCPESunat(id_redeemed_sale) {
  EventBus.$emit('ModalCPESunatShow',id_redeemed_sale);
}

function SendXML(id_redeemed_sale) {
  this.isLoading = true;
  let me = this;
  let url = this.url_base + "redeemed-sale/send-xml/" + id_redeemed_sale;
  axios({
    method: "get",
    url: url,
    headers: { token: this.token, module: this.module,role: 1 },
    ...
    .then(function (response) {
      if (response.data.status == 200) {
        for (var i = 0; i < me.data_table.length; i++) {
          if (me.data_table[i].id_redeemed_sale == id_redeemed_sale) {
...
// Confirmar eliminar
function ConfirmDeleteSale(id_redeemed_sale) {
  Swal.fire({
    ...
    if (result.value) {
      this.DeleteSale(id_redeemed_sale);
    }
  });
}
...
function showModalSaleLow(index) {
  ...
  this.sale_low.id_redeemed_sale = this.data_table[index].id_redeemed_sale;
