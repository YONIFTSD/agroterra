  ...
  <div>
    <CRow>
      <CCol col>
        <CCard>
          <CCardHeader>
            <strong> Modulo Cuentas por Pagar - Nuevo</strong>
          </CCardHeader>
          <CCardBody>
...
function AddAccountPay(me) {
  ...
  me.account_pay.id_shopping = 0;
  me.account_pay.id_provider = me.provider.id;
  let url = me.url_base + "account-pay/add";
  ...
  axios({
    ...
    .then(function (response) {
      if (response.data.status == 201) {
          me.account_pay.id_shopping = 0;
          me.account_pay.id_user = '';
          me.account_pay.id_establishment = '';
          me.account_pay.id_provider = '';
          me.account_pay.type_invoice = '01';
          me.account_pay.serie = '';
          me.account_pay.number = '';
          me.account_pay.broadcast_date = moment(new Date()).local().format("YYYY-MM-DD");
          me.account_pay.expiration_date = moment(new Date()).local().format("YYYY-MM-DD");
          me.account_pay.coin = 'PEN';
          me.account_pay.reason = '';
          me.account_pay.total = (0).toFixed(2);
          me.account_pay.balance = (0).toFixed(2);
          me.account_pay.fee = (0).toFixed(2);
          me.account_pay.state = 1;
          me.provider = null;

          Swal.fire({ icon: 'success', text: 'Se ha registrado la cuenta por pagar', timer: 3000,})
...
function Validate() {
 ...
 let me = this;
  ...
  Swal.fire({
    title: 'Esta seguro de registrar la cuenta por pagar?',
    icon: 'warning',
