  ...
  <div>
    <CRow>
      <CCol col>
        <CCard>
          <CCardHeader>
            <strong> Modulo Nota de Crédito/Débito - Listar</strong>
          </CCardHeader>
          <CCardBody>
            <b-row>
              <b-col sm="12" md="5">
                  <b-form-group>
                    ...
                    <v-select @input="ListCreditDebitNote" placeholder="Todos" class="w-100" :filterable="false" label="name" v-model="provider" @search="SearchProvider" :options="providers"></v-select>
                  </b-form-group>
                </b-col>
                ...
                <b-col sm="6" md="1">
                  <b-form-group label=".">
                    <b-link  v-if="Permission('CreditDebitNoteAdd')" class="btn form-control btn-primary"  :to="{ path: '/nota-compra/nuevo' }" append ><i class="fas fa-file"></i></b-link>
                  </b-form-group>
                </b-col>
            </b-row>
              ...
              <table class="table table-hover table-bordered">
                ...
                <tbody v-for="(item, it) in data_table" :key="it">
                  <tr>
                    ...
                    <td class="text-left"> {{ CodeInvoice(item.type_invoice)  + ' '+item.serie + '-'+item.number }}</td>
                    ...
                    <td class="text-center">
                      <b-dropdown bloque size="sm" text="Acciones" right>
                        <b-dropdown-item v-if="Permission('CreditDebitNoteEdit')"  @click="EditCreditDebitNote(item.id_credit_debit_note)">Editar</b-dropdown-item>
                        <b-dropdown-item v-if="Permission('CreditDebitNoteView')"  @click="ViewCreditDebitNote(item.id_credit_debit_note)">Ver</b-dropdown-item>
                        <b-dropdown-item v-if="Permission('CreditDebitNoteDelete')" @click="ConfirmDeleteCreditDebitNote(item.id_credit_debit_note)">Eliminar</b-dropdown-item>
                      </b-dropdown>
                    </td>
                  </tr>
                </tbody>
...
// Editar usuario
function EditCreditDebitNote(id_credit_debit_note) {
  this.$router.push({
    name: "CreditDebitNoteEdit",
    params: { id_credit_debit_note: je.encrypt(id_credit_debit_note) },
  });
}

// Ver Usuario
function ViewCreditDebitNote(id_credit_debit_note) {
  this.$router.push({
...
// Confirmar eliminar
function ConfirmDeleteCreditDebitNote(id_credit_debit_note) {
  Swal.fire({
    title: "Esta seguro de eliminar la nota de crédito/débito?",
    ...
    if (result.value) {
      this.DeleteCreditDebitNote(id_credit_debit_note);
    }
  });
}

// eliminar usuario
function DeleteCreditDebitNote(id_credit_debit_note) {
  let me = this;
  let url = this.url_base + "credit-debit-note/delete/" + id_credit_debit_note;
  axios({
    method: "delete",
    url: url,
    headers: {
    ...
    .then(function (response) {
      if (response.data.status == 200) {
        ...
        for (var i = 0; i < me.data_table.length; i++) {
          if (me.data_table[i].id_credit_debit_note == id_credit_debit_note) {
            me.data_table.splice(i, 1);
            break;
          }
        }
        Swal.fire({ icon: 'success', text: 'La nota de crédito/débito ha sido eliminado', timer: 3000,})
